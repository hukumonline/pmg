...
/**
 * Description of GlobeController
 *
...
class Misc_GlobeController extends Zend_Controller_Action
{
    function tentangkamiAction()
    {
        ...
        foreach ($rowset as $row)
        {
            ...
            $data[$content][1] = App_Model_Show_CatalogAttribute::show()->getCatalogAttributeValue($row->guid,'fixedTitle');
...
//        $sDir = 'uploads/images';
    ...
    function printedocAction()
    {
        ...
        if (isset($rowset))
            ...
            $this->view->title = $title;
            $this->view->shortTitle = $rowset['shortTitle'];
            $this->view->content = $content;
    ...
    function sendshareAction()
    {
        ...
        $rowset = App_Model_Show_Catalog::show()->getCatalogByGuid('lt49841c739c980'); // --> kutu_emailconfirm untuk share article

        if (isset($rowset))
                //$title = App_Model_Show_CatalogAttribute::show()->getCatalogAttributeValue($rowset['guid'],'fixedTitle');
                $mailcontent = App_Model_Show_CatalogAttribute::show()->getCatalogAttributeValue($rowset['guid'],'fixedContent');
    ...
    private function getcatalogshare($catalogGuid)
    {
        ...
        $array_hari = array(1=>"Senin","Selasa","Rabu","Kamis","Jumat","Sabtu","Minggu");
        $hari = $array_hari[date("N",strtotime($rowset['createdDate']))];
        $publish = $hari . ', '. date("d F Y",strtotime($rowset['createdDate']));
