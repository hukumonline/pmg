...
class App_Model_Show_Order extends App_Model_Db_DefaultAdapter
{
    ...
    {
        $db = parent::_getDefaultAdapter();
        $query = $db->query("SELECT KO.*,KOS.ordersStatus, COUNT(itemId) AS countTotal,KU.* FROM ((KutuOrder AS KO Left join KutuOrderDetail AS KOD ON KOD.orderId=KO.orderId)
                                LEFT JOIN KutuUser AS KU ON KU.guid = KO.userId)
                                LEFT JOIN KutuOrderStatus AS KOS ON KOS.orderStatusId = KO.orderStatus
